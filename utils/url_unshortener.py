...
def bulkUnshorten(urls, workers=20, REQUEST_TIMEOUT=10, HOPS_LIMIT=5):
    ...
    log.info(f"Bulk Unshorten 2 called with {len(urls)} urls. Num workers specified: {workers}")
    log.info(f"Hops limit is {HOPS_LIMIT} and request Timeout Seconds = {REQUEST_TIMEOUT}")
    ...
    if (isinstance(urls, list)):
        ...
        for url in url_objects:
            ...
            try:
                ...
                log.info(f"Error Unshortening: InvalidURL on {url}. Error {e}")
                url_dict['success'] = False
            except MissingSchema as e:
                log.info(f"Error Unshortening: MissingSchema on {url}. Error {e}")
                url_dict['success'] = False
            finally:
